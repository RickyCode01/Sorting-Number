...
void Merge(int *A, int a, int b, int c){
    ...
    int n1 = b-a; // lenght of left array
    ...
    for(size_t k=a; k<c; k++){ // copy values in subarrays
        // printArray(A, c);
        // printf("i:%d, j:%d\n", i, j);
        if(L[i] <= R[j]){ // select first value of left
            A[k] = L[i]; // write value into array 
            i++; 
        }else{ // select first value of right
            A[k] = R[j];
            j++;
        }
    }
    printArray(A, a, c, false);
}
